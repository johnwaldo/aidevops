...
rt_detect_installed() {
	...
	while [[ $i -lt $_RT_COUNT ]]; do
		bin="${_RT_BINARY[$i]}"
		# Use type -P to find real filesystem executables only.
		# command -v matches shell builtins/keywords (e.g., "continue") causing
		# false positives — type -P returns only executables in PATH.
